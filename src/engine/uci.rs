...
#[async_trait]
impl<T: Io + Send> Engine for Uci<T> {
    type Error = UciError;
...
mod tests {
    ...
    fn engine_is_lazily_initialized_with_the_options_configured(
        ...
        assert!(rt.block_on(uci.best(&pos)).is_ok());
    }
    ...
    fn initialization_can_fail(pos: Position, e: io::Error) {
        ...
        let mut uci = Uci::new(io);
        assert_eq!(
            rt.block_on(uci.best(&pos)).map_err(|UciError(e)| e.kind()),
            Err(kind)
        );
    }
    ...
    fn play_instructs_engine_to_make_move(l: Limits, pos: Position, m: Move) {
        ...
        assert_eq!(rt.block_on(uci.best(&pos))?, m);
    }
